## Quiz: Object Oriented Programming: Grade 100%
...
The line `city1.elevation > return_city.elevation` should be `city1.elevation > return_city.elevation` in the first evaluation, and in the other similar evaluations, it should be `city2.elevation > return_city.elevation` and `city3.elevation > return_city.elevation`. This is because it's comparing the elevation of each city with the elevation of the city that has been registered as the one with the highest elevation up to that point in `return_city`.

In other words, it's comparing the elevations of the cities (city1, city2, and city3) with the elevation of the city currently considered to have the highest elevation (return_city). If a city's elevation is higher than return_city's elevation, then return_city is updated to contain the values of that city because this new city has the highest elevation up to that point and meets the minimum population criterion. This is done in each of the three evaluations to determine which of the three cities meets the criteria and has the highest elevation.

Each of the three cities is evaluated in succession. If a city meets the minimum population criteria and its elevation is higher than return_city's elevation (which was initialized as a fictitious city with an elevation of 0), then that city is assigned to return_city.
